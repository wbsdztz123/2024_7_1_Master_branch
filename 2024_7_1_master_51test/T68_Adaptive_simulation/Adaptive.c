                ...
                && (fabs((Message_VehicleMsg.Velocity / 3.6f) - CalibrationPara.Velocity) < 3.0f)) // 3
            {
                ...
                for (i = 0; i < PeakList->point_count; i++) {
                    ...
                    if (Calibration_flag) {
                        ...
                        float X = PeakList->term[i].range * cos((RadarPara.InstallAngle - azimuth_deg) * PI / 180);
                        float Y = PeakList->term[i].range * sin((RadarPara.InstallAngle - azimuth_deg) * PI / 180);
...
uint8_t Rang_judge(const or_point_cloud_format_t *PeakList)
{
    ...
    for (i = 0; i < PeakList->point_count; i++)
    {
        ...
        if (result)
        {
            float32_t temp_Ydata = PeakList->term[i].range * sin((RadarPara.InstallAngle - (PeakList->term[i].azimuth * 180 / PI)) * PI / 180);
            if (temp_Ydata < 2.01f) {
                rang0_2++;
...
uint8_t Body_Posture_Detection(void)
{
    ...
    if (velocity < Calibration_MinVelocity) {
        CalibrationPara.Driving_Profile = 0x01;
    }

    if (velocity > Calibration_MaxVelocity) {
...
uint8_t CAL_Target_Filtering(const or_point_cloud_format_t *PeakList, uint8_t i)
{
    ...
    if (PeakList->term[i].range > CalibrationRangeMin 
        ...
        && (PeakList->term[i].azimuth * 180 / PI) > -10.0f
