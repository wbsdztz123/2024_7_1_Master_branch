...
功能：
	核心工作区： 存储当前正在运行的"操作系统内核"
                        1.进程管理，调度，创建销毁，上下文切换； 
                        2.文件系统，磁盘空间管理，数据缓存，文件读写接口； 
                        3.网络通信，TCP/IP，套接字接口，数据包路由； 
                        4.设备驱动，硬件抽象层，中断处理，设备IO控制)、应用程序代码（从硬盘加载而来）、应用程序处理的数据。
    
	包含栈和堆： 栈和堆是操作系统在 RAM 中为每个进程/线程划分出来的逻辑区域。栈和堆是 RAM 的一部分！
	易失性： RAM 是 易失性存储器 (Volatile Memory)。断电后，存储在 RAM 中的所有数据（包括栈和堆的内容）都会丢失。
    管理：由操作系统的内存管理单元 (MMU - Memory Management Unit，通常是 CPU 的一部分) 进行管理。
