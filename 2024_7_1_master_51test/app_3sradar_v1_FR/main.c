    ...
    while (fgets(buffer, sizeof(buffer), output_fp)) //this is line
    {
        ...
        else
        {
            ...
            {   
                ...
                if((list == Frame_number)||(list == Serial_number)||(list == Range)|| (list == Doppler)||(list == Azimuth)||(list == Elev)||(list == Snr)||(list == Vel)||(list == Yaw)||(list == Steering)||(list == Cur))
                {
                    ...
                    {   
                        ...
                        break;
                        case Azimuth:
                            Calibration_Message.or_point_cloud_format_t.term[point_id].azimuth = DEG_TO_RAD((float)atof(token));
                        break;
                        case Elev:
                        Calibration_Message.or_point_cloud_format_t.term[point_id].elevation = DEG_TO_RAD((float)atof(token));
                        break;
                        case Snr:
...
void Calibration_runing_task(void)
{
    ...
    while (1)
    {
        //printf("Calibration_runing_task\n");
        ...
        Calibration_Required_data();
            switch(CAL_MODE)
            {
                ...
                case CALIBRATION_RUNING:
                    adaptive_flow_control_func(&Calibration_Message.or_point_cloud_format_t);
                break;
                default:
                    break;
            }
